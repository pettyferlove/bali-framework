...
import com.github.bali.core.framework.annotation.RedissonLock;
...
public class RedissonLockAspect {
    ...
    public Object around(ProceedingJoinPoint joinPoint, RedissonLock redissonLock) throws Throwable {
        ...
        String template = redissonLock.value();
        ...
        boolean look;
        if (redissonLock.leaseSeconds() == -1L) {
            look = redissonLockService.look(redissonLock.waitSeconds(), key);
        } else {
            look = redissonLockService.look(redissonLock.waitSeconds(), redissonLock.leaseSeconds(), key);
        }
        if (!look) {
            throw new RuntimeException("当前业务正在操作中，请稍后再试");
        }
        try {
            return joinPoint.proceed(args);
        } finally {
            redissonLockService.unLook(key);
        }
